...
enum S3DocArchives {
    ...
    static func fetch(prefix: String,
                      ...
                      awsSecretAccessKey: String) async throws -> [DocSet] {
    ...
    static let folder = Parse(DocSet.init) {
        pathSegment
        pathSegment
        pathSegment
        "documentation/"
...
private extension S3 {
    func listFiles(in bucket: String, key: S3DocArchives.S3StoreKey, delimiter: String? = nil) async throws -> [S3DocArchives.S3FileDescriptor] {
        try await listFiles(in: bucket, key: key, delimiter: delimiter).get()
    }

    func listFiles(in bucket: String, key: S3DocArchives.S3StoreKey, delimiter: String? = nil) -> EventLoopFuture<[S3DocArchives.S3FileDescriptor]> {
        ...
        return self.listObjectsV2Paginator(request, []) { accumulator, response, eventLoop in
            let files: [S3DocArchives.S3FileDescriptor] = response.contents?.compactMap {
                guard let key = $0.key,
                      let lastModified = $0.lastModified,
                      let fileSize = $0.size else { return nil }
                return S3DocArchives.S3FileDescriptor(
                    file: S3DocArchives.S3File(bucket: bucket, key: key),
